...
namespace Unicorn.UnitTests.UI.Tests.Web
{
    ...
    public class WebPageObjectTests : WebTestsBase
    {
        ...
        public void TestPageObjectControlsSearch()
        {
            ...
            Assert.That(page.JqCheckbox.GetType(), Is.EqualTo(typeof(Checkbox)));
        ...
        {
            //JqueryCheckboxRadioPage page = NavigateToPage<JqueryCheckboxRadioPage>();
            CustomCheckbox cbox = NavigateToPage<JqueryCheckboxRadioPage>().CheckboxesCustomList[0];
            Assert.IsTrue(cbox.GetAttribute("class").Contains("ui-checkboxradio-label"));
            Assert.IsTrue(cbox.Label.GetAttribute("class").Contains("ui-checkboxradio-icon"));
        }
